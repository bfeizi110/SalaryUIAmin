...
import { ReportDesignStimulAttr, setReportDesignStimulAttr } from 'src/app/main/pages/global-attr'

const Controller = 'ReportDesignStimul'

@Component({
  selector: 'report-design-stimul',
  templateUrl: './report-design-stimul.component.html',
  styleUrls: ['./report-design-stimul.component.scss']
})
export class ReportDesignStimulComponent {
  ...
  getAttr() {
    this.showGrid = false
    let Attr = ReportDesignStimulAttr()
    !Attr
  ...
  setAttr(attr, type?) {
    ...
    type == 'toLocal' ? setReportDesignStimulAttr(attr) : null
    this.getSelect()
  }
  ...
  getSelect() {
    ...
    this.service.getSelect(Controller, this.parentId).subscribe((res: any) => {
      ...
      this.showGrid = true
      setTimeout(() => {
        this.service.scrollToElement('report-stimul')
      }, 200); 
    })
  }
