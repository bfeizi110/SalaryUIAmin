...
export class WorkFlowFormComponent {
  ...
  post(fakeFormValue) { this.service.post(`${Controller}/Create`, fakeFormValue).subscribe((res: any) => { res && res.IsSuccess ? this.done.emit(res.Data) : null })}

  put(fakeFormValue) { this.service.post(`${Controller}/Update`, fakeFormValue).subscribe((res: any) => { res && res.IsSuccess ? this.done.emit(res.Data) : null })}
