...
const Controller = 'FormDetailsNew'

@Component({
  selector: 'form-detailsnew-form',
  templateUrl: './form-detailsnew-form.component.html',
  styleUrls: ['./form-detailsnew-form.component.scss']
})
export class FormDetailsNewFormComponent {
  ...
  setForm() {
    ...
    this.form = this.formBuilder.group({
      ...
      ShowSize_Fld: [{ value: null, disabled: Validation.disable('ShowSize_Fld') }, Validation.setValidator('ShowSize_Fld')],
  ...
  changeDynamicFormType() {
    // if (this.DynamicFormType == 100957)
    // {
  ...
  changeType() {
    if (this.form.controls.Type_Fld.value != 101201) {
      ...
      this.form.controls.ShowSize_Fld.clearValidators()
      this.form.controls.ShowSize_Fld.updateValueAndValidity();
      this.formObj.ShowSize_Fld ? this.formObj.ShowSize_Fld.require = false : null
      this.form.controls.EntityPropertyID_Fld.patchValue(null)
      this.form.controls.ShowSize_Fld.patchValue(null)
    }
    else {
