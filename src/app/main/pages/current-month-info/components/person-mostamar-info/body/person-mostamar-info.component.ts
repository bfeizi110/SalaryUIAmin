...
import { PersonMostamarInfoAttr, setPersonMostamarInfoAttr } from 'src/app/main/pages/global-attr'

@Component({
  selector: 'app-person-mostamar-info',
  templateUrl: './person-mostamar-info.component.html',
  styleUrls: ['./person-mostamar-info.component.scss']
})
export class PersonMostamarInfoComponent {
  ...
  async onFilter(dto) {
    ...
    this.service.post(this.controller == 'PersonMostamarBaz' ? 'PersonBaz/GetFilterSelectBazMov' : `Person/GetFilterSelect`, dto).subscribe((res: any) => {
  ...
  async onSelectPersonnel(id) {
    ...
    this.showGrid = false
    this.showForm = false
    this.cardexOptions.ComboType = `${this.controller}/GetPersonMostamarCombo`
  }
  ...
  getAttr() {
    return new Promise(resolve => {
      ...
      if (!PersonMostamarInfoAttr) {
        ...
        this.setAttr(PersonMostamarInfoAttr)
        return resolve(true)
      }
    })
  }
  ...
  setAttr(attr, type?) {
    ...
    type == 'toLocal' ? setPersonMostamarInfoAttr(attr) : null
  }
  ...
  getSelect() {
    ...
    this.service.getSelect(this.controller, this.PID).subscribe((res: any) => {
      ...
      this.showGrid = true
      setTimeout(() => {
        this.service.scrollToElement('person-mostamar-tab')
      }, 200); 
    })
  }
  ...
  constructor(private service: GridFormService, private contextMenuService: ContextMenuService, router: Router) {
    ...
    if (this.pathUrl.includes('baz-subsystem')) this.controller = 'PersonMostamarBaz'
    else this.controller = 'PersonMostamar'
