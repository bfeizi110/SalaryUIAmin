...
import { TadrisPayAttr, setTadrisPayAttr } from 'src/app/main/pages/global-attr'

const Controller = 'Tadris'

@Component({
  selector: 'tadris-pay',
  templateUrl: './tadris-pay.component.html',
  styleUrls: ['./tadris-pay.component.scss']
})
export class TadrisPayComponent {

  @Input() parentId: number
  @Input() rowData: any
  @Input() formTypeParent: string
  @Output() submitedOnGrid = new EventEmitter()
  @Output() PayTypeChanged = new EventEmitter()
  ...
  getAttr() {
    this.showGrid = false
    let Attr = TadrisPayAttr()
    !Attr
      ? this.service.get(`${Controller}/GetAttributeTadrisDetail`).subscribe((res: any) => this.setAttr(res.Data, 'toLocal'))
      : this.setAttr(Attr)
  }

  setAttr(attr, type?) {
    this.gridOption.columnDefs = attr
    !this.rowData || this.formTypeParent == 'Add' ? this.gridOption.rowData = [{Id: 0, TadrisPayTypeDesc_Fld: null, Vahed_Fld: null, Hour_Fld: null }] : this.gridOption.rowData = this.rowData
    this.formObj = attr.EntityAttribute
    type == 'toLocal' ? setTadrisPayAttr(attr) : null
  ...
  delete(event) {
    ...
    this.gridOption.rowData.length == 0 ? this.gridOption.rowData = [{ Id: 0, TadrisPayTypeDesc_Fld: null, Vahed_Fld: null, Hour_Fld: null }] : null
    this.showForm = false
