...
export class CustomAgGridComponent implements OnInit {
  ...
  setGridOption() {
    if (!this.isModal) {
      ...
      {
 this.gridOption.defaultColDef = defaultColDefSimple;
      } 

      //this.framework = this.frameworkComponentsSimple;
       (this.gridOption as any).frameworkComponents = this.frameworkComponents;
  }
  ...
  onCellKeyPress(e) {
    if (e.event.key == 'Enter') this.rowClicked(e);
  }

  selectingRowCheckbox() {
    if (
      this.customGridOption.selectedRowCheckboxData &&
      this.customGridOption.selectedRowCheckboxData.length != 0
    ) {
      this.gridApi.forEachNode((node) =>
        this.customGridOption.selectedRowCheckboxData.forEach((e) =>
          node.setSelected(node.data === e)
        )
      );
    }
  }
  ...
  rowClicked(event) {
    this.gridApi.deselectAll();
    event?.column?.colId !== 'actions' && this.customGridOption.rowClicked
      ? this.customGridOption.rowClicked(event)
      : null;
  }

  rowDoubleClicked(event) {
    event.data && this.customGridOption.rowDoubleClicked
      ? this.customGridOption.rowDoubleClicked(event)
      : null;
  }
  ...
  setGrid() {
    this.customGridOption.rowData[0].Id != 0
    ...
    const filteredColumnKey = Object.keys(columnObj)
      .filter((key) => keyArray.includes(key))
      .reduce((obj, key) => {
        if (columnObj[key] != null) obj[key] = columnObj[key];
        return obj;
      }, {});

    const filteredColumnValue = Object.values(filteredColumnKey).filter(
      (a: any) => !a.hiddenInGrid
    );
    ...
    if (!this.isModal)
      this.gridOption.columnDefs.map((a: any) => {
        ...
        a.type == 'bool'
          ? (a.cellRendererFramework = CheckboxRendererComponent)
          : (controller == 'PersonMostamarBaz' ||
              controller == 'PersonSavingInfoBaz' ||
              controller == 'PersonDebtInfoBaz' ||
              controller == 'PersonInsureInfoBaz' ||
              controller == 'CommissionBaz') &&
            a.field == 'PID'
      ...
      this.gridOption.columnDefs.map((a: any) => {
        (a.isModal = this.isModal),
          a.width != 0 ? (a.width = a.width) : (a.flex = 1);
        a.type == 'int'
          ? (a.valueFormatter = function (params) {
              if (params.value)
    ...
    actions
      ? this.gridOption.columnDefs.push({
          ...
          headerComponentParams: {
            actions: actions,
            actionAccess: this.customGridOption.columnDefs.EntityAccess,
          },
          cellRenderer: ButtonRendererComponent,
          cellRendererParams: {
            hideLogs: this.hideLogs,
            actions: actions,
            miniLogUrl: this.customGridOption.miniLogUrl,
            controllerName: this.customGridOption.controllerName,
            tableName:
              this.customGridOption.columnDefs.EntityAttribute.EntityName,
            actionAccess: this.customGridOption.columnDefs.EntityAccess,
          },
          filter: false,
          lockPinned: true,
    ...
    this.customGridOption.checkboxSelection
      ? this.setCheckboxSelection()
      : delete this.gridOption.defaultColDef.headerCheckboxSelection;
    this.setRowHeight();
    let entityName: string =
      this.customGridOption.columnDefs.EntityAttribute.EntityName;
    if (
      this.customGridOption.columnDefs.EntityDAccess &&
      (entityName == 'Tbl_Prsinfo' ||
        entityName == 'PersonMonthInfo_Tbl' ||
        entityName == 'PersonWorkAddingInfo_Tbl')
    )
      this.setContextMenu();

    if (
      (this.masterDetail !== false && controller == 'PersonMostamarBaz') ||
      controller == 'PersonSavingInfoBaz' ||
      controller == 'PersonDebtInfoBaz' ||
      controller == 'PersonInsureInfoBaz' ||
      controller == 'CommissionBaz'
    ) {
      let rowsById0: any[] = this.customGridOption.rowData.filter(
        (row) => row.PersonSelectMovDto[0].Id == 0
      );
      let keyArray = [];
      ...
      this.gridOption.isRowMaster = (dataItem) => {
        return dataItem
          ? dataItem.PersonSelectMovDto &&
              dataItem.PersonSelectMovDto.length > 0
          : false;
      };
      this.gridOption.detailCellRendererParams = {
        ...
        getDetailRowData: (params) =>
          params.successCallback(params.data.PersonSelectMovDto),
      };
    }
  }
  ...
  rowClickedDetail(event) {
    this.customGridOption.rowClickedDetail(event);
  }
  ...
  setCheckboxSelection() {
    ...
    this.formType != ''
      ? Object.assign(this.gridOption.defaultColDef, {
          headerCheckboxSelection: setFirstColumn,
        })
      : null;
  }
  ...
  cellContextMenu(event) {
    ...
    this.Id =
      controller == 'PersonMonthInfo' || controller == 'PersonWorkAddingInfo'
        ? event.data.PersonID_Fld
        : event.data.Id;
  }

  setContextMenu() {
    let menuIds: number[] =
      this.customGridOption.columnDefs.EntityDAccess.split(',');
    let menuList: any[] = JSON.parse(sessionStorage.getItem('MenuList'));
    if (menuList) {
      menuList.length > 0
        ? menuList.forEach((menu) =>
            menuIds.forEach((menuId) =>
              menu.Id == menuId ? this.menuListSelected.push(menu) : null
            )
          )
        : null;
      this.menuListSelected.forEach((menu) =>
        this.contextMenuList.push({
          name: menu.CodeDesc_Fld,
          action: this.rowClickedContexMenu.bind(this, menu),
          icon: createFlagImg(menu.Icon),
        })
      );
      this.contextMenuList.push({
        name: 'مشاهده فیش حقوقی',
        action: this.rowClickedContexMenu.bind(this, 'fish'),
        icon: createFlagImg('fish'),
      });
    }
  }
  ...
  setFocus() {
    ...
    if (controller == 'PersonWorkAddingInfo')
      this.gridApi.forEachNode((node) =>
        node.data.PersonID_Fld == this.focusField
          ? (rowIndex = node.rowIndex)
          : null
      );
    else
      this.gridApi.forEachNode((node) =>
        node.data.Id == this.focusField ? (rowIndex = node.rowIndex) : null
      );
    this.gridApi.ensureIndexVisible(rowIndex);
    setTimeout(() => {
      this.gridApi.setFocusedCell(rowIndex, 'PID');
    }, 400);
  }
  ...
  onPageSizeChanged(index) {
    this.divGrid
      ? (this.divGrid.nativeElement.firstElementChild.style.height =
          Number(index) * 20.5 + 75 + 'px')
      : null;
  }
  ...
  exportToPDF() {
    printDoc(this.printParams, this.gridApi, this.gridOption.columnApi);
  }
  ...
  ngOnChanges(UpdatedValue: any): void {
    if (
      UpdatedValue['signalRChange'] &&
      UpdatedValue['signalRChange'].currentValue &&
...
function createFlagImg(flag) {
  return `<img border="0" width="15" src="../../../../../../assets/img/menu-icons/${flag}.png">`;
}
